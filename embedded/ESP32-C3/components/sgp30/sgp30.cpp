...
SGP30::SGP30(i2c_port_t i2c_num, gpio_num_t sda_gpio, gpio_num_t scl_gpio, uint32_t clk_speed)
    : i2c_num(i2c_num), sda_gpio(sda_gpio), scl_gpio(scl_gpio), clk_speed(clk_speed) {}
...
esp_err_t SGP30::write_command(const uint8_t *cmd, size_t cmd_len)
{
    ...
    i2c_master_write_byte(handle, (I2C_ADDRESS << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write(handle, cmd, cmd_len, true);
    i2c_master_stop(handle);
    esp_err_t err = i2c_master_cmd_begin(i2c_num, handle, pdMS_TO_TICKS(1000));
