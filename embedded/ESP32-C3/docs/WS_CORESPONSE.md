...
{
    "code": 0,          // 0: 查询, 1: 设定
    "item": "device",   // 操作模块（device / light / sensor / scale）
    "key": "version",   // 具体属性名
...
| `item` | string | 是 | 操作模块，支持 `device`、`light`、`sensor`、`scale` |
...
scale 模块支持查询和设定，用于控制 HX711 称重传感器。

| key | 操作 | values 取值 | 说明 |
|-----|------|-------------|------|
| `weight` | 查询 | - | 读取当前重量（克） |
| `status` | 查询 | - | 查询传感器状态（就绪/标定/量程） |
| `full_scale` | 查询/设定 | 数字（克），0 表示不限量程 | 查询或设置量程上限 |
| `tare` | 设定 | 任意值 | 去皮（空载归零） |
| `calibrate` | 设定 | 砝码重量（克），正数 | 放入已知砝码后自动标定 |

#### 3.4.1 查询重量

```json
{"code":0,"item":"scale","key":"weight","values":""}
```
响应（正常）：
```json
{"code":1,"msg":"OK.","key":"weight","values":"{\"weight_g\":1234.5,\"overload\":false,\"full_scale_g\":5000}"}
```
响应（超量程）：
```json
{"code":1,"msg":"OK.","key":"weight","values":"{\"weight_g\":5120.3,\"overload\":true,\"full_scale_g\":5000}"}
```
响应（未标定）：
```json
{"code":1,"msg":"OK.","key":"weight","values":"{\"weight_g\":0.0,\"status\":\"uncalibrated\"}"}
```

#### 3.4.2 查询状态

```json
{"code":0,"item":"scale","key":"status","values":""}
```
响应：
```json
{"code":1,"msg":"OK.","key":"status","values":"{\"ready\":true,\"calibrated\":true,\"full_scale_g\":5000}"}
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `ready` | bool | HX711 传感器是否就绪 |
| `calibrated` | bool | 是否已完成标定 |
| `full_scale_g` | float | 当前量程上限（克），0 表示不限制 |
...
```json
{"code":1,"item":"scale","key":"tare","values":""}
```
响应（成功）：
```json
{"code":1,"msg":"去皮成功.","key":"tare","values":"ok"}
```
响应（失败）：
```json
{"code":0,"msg":"去皮失败（HX711未就绪）","key":"tare","values":""}
```
...
1. 先执行去皮（空载归零）
2. 在称盘上放置已知重量的砝码
3. `values` 传入砝码的实际重量（克）

```json
{"code":1,"item":"scale","key":"calibrate","values":100}
```
响应（成功）：
```json
{"code":1,"msg":"标定成功.","key":"calibrate","values":"100.0"}
```
响应（失败）：
```json
{"code":0,"msg":"标定失败（请确认已放上砝码且HX711就绪）","key":"calibrate","values":""}
```

#### 3.4.5 设置量程

`values` 为数字时表示设置量程上限（克），设为 `0` 则不限制量程。

```json
{"code":1,"item":"scale","key":"full_scale","values":5000}
```
响应：
```json
{"code":1,"msg":"量程设置成功.","key":"full_scale","values":"5000"}
```

**查询当前量程**：
```json
{"code":0,"item":"scale","key":"full_scale","values":""}
```
响应：
```json
{"code":1,"msg":"OK.","key":"full_scale","values":"{\"ready\":true,\"calibrated\":true,\"full_scale_g\":5000}"}
```

#### 3.4.6 典型标定流程

1. 确保称盘为空
2. 执行去皮：`{"code":1,"item":"scale","key":"tare","values":""}`
3. 放上已知砝码（如 100g）
4. 执行标定：`{"code":1,"item":"scale","key":"calibrate","values":100}`
5. （可选）设置量程：`{"code":1,"item":"scale","key":"full_scale","values":5000}`
6. 查询验证：`{"code":0,"item":"scale","key":"weight","values":""}`
...
| 去皮失败 | `{"code":0,"msg":"去皮失败（HX711未就绪）","key":"tare","values":""}` |
| 砝码重量无效 | `{"code":0,"msg":"砝码重量无效，需为正数","key":"calibrate","values":""}` |
| 标定失败 | `{"code":0,"msg":"标定失败（请确认已放上砝码且HX711就绪）","key":"calibrate","values":""}` |
| 量程值无效 | `{"code":0,"msg":"量程值无效","key":"full_scale","values":""}` |
| 未知的 scale 键 | `{"code":0,"msg":"未知的scale键","key":"xxx","values":""}` |
...
4. **后台采集**：设备每分钟自动采集全部 7 个传感器数据并聚合上报到服务端。`sensor` / `scale` 查询接口为独立的按需读取通道，两者互不影响。
