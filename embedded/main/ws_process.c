    ...
    if (json != NULL)
    {
        ...
        if (cJSON_IsNumber(code_item) && cJSON_IsString(item_item) && cJSON_IsString(key_item) && (cJSON_IsString(values_item) || cJSON_IsNumber(values_item)))
        {
            ...
            if (code == 0)
            {
                ...
                if (strcasecmp(item, "status") == 0)
                {
                    if (strcasecmp(key, "status") == 0)
                    {
                        ...
                        if (device.status == DEVICE_ON_STREAM)
                            status_str = "stream";
                        else if (device.status == DEVICE_STANDBY)
                            status_str = "standby";
                        else
                            status_str = "unknown";
                        snprintf(values_buf, sizeof(values_buf), "%s", status_str);
                        success = 1;
                        snprintf(msg, sizeof(msg), "OK.");
                    }
                    else
                    {
                        snprintf(msg, sizeof(msg), "未知的状态键");
                    }
                }
                else if (strcasecmp(item, "camera") == 0)
                {
                    sensor_t *s = esp_camera_sensor_get();
                    if (s == NULL)
                    {
                        snprintf(msg, sizeof(msg), "相机未初始化");
                    }
                    else
                    {
                        ...
                        {
                            // 获取当前帧大小
                            framesize_t fs = s->status.framesize;  // 实际字段可能为 s->status.framesize
                            const char *fs_str = "";
                            switch (fs)
                            {
                                case FRAMESIZE_128X128: fs_str = "FRAMESIZE_128X128"; break;
                                case FRAMESIZE_240X240: fs_str = "FRAMESIZE_240X240"; break;
                                case FRAMESIZE_320X320: fs_str = "FRAMESIZE_320X320"; break;
                                case FRAMESIZE_VGA: fs_str = "FRAMESIZE_VGA"; break;
                                case FRAMESIZE_SVGA: fs_str = "FRAMESIZE_SVGA"; break;
                                case FRAMESIZE_HD: fs_str = "FRAMESIZE_HD"; break;
                                case FRAMESIZE_FHD: fs_str = "FRAMESIZE_FHD"; break;
                                default: fs_str = "unknown";
                            }
                            snprintf(values_buf, sizeof(values_buf), "%s", fs_str);
                            success = 1;
                            snprintf(msg, sizeof(msg), "OK.");
                        }
                        else if (strcasecmp(key, "jpeg_quality") == 0)
                        {
                            int quality = s->status.quality;  // 实际字段可能为 s->status.quality
                        ...
                        {
                            int hmirror = s->status.hmirror;  // 实际字段可能为 s->status.hmirror
                            snprintf(values_buf, sizeof(values_buf), "%d", hmirror);
                            success = 1;
                            snprintf(msg, sizeof(msg), "OK.");
                        }
                        else if (strcasecmp(key, "set_vflip") == 0)
                        {
                            int vflip = s->status.vflip;  // 实际字段可能为 s->status.vflip
                            snprintf(values_buf, sizeof(values_buf), "%d", vflip);
                            success = 1;
                            snprintf(msg, sizeof(msg), "OK.");
                        }
                        else
                ...
                {
                    snprintf(msg, sizeof(msg), "不支持的item");
                }
            ...
            else
            {
                ...
                char return_data[128];
                snprintf(return_data, sizeof(return_data),
                         "{\"code\":0,\"msg\":\"无效的code\",\"key\":\"%s\",\"values\":\"\"}",
                         key);
                WebsocketSendText(return_data, strlen(return_data));
            }
        }
