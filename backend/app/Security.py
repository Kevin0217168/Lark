...
def VerifyToken(db: Db.Session, token: str, isAllowCookie: bool) -> Db.M_Users:
    if isAllowCookie:
...
async def GetCurrentUser(
    ...
    return VerifyToken(db, token, False)


async def GetCurrentUserByCookie(
    long_token: Annotated[str | None, Cookie()] = None,
    db: Db.Session = Depends(Db.GetDb("GetCurrentUserByCookie")),
):

    return VerifyToken(db, long_token, True)
