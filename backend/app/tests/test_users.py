...
# 导入待测试的路由和依赖模块
from userapi.User import router
import Db
import Security
...
def test_get_user_by_id(mock_get_users, client, app):
    """测试根据ID获取单个用户（成功）"""
    fake_user = [{"id": 1, "username": "user1", "email": "user1@example.com"}]
...
@patch("Db.GetUsers")
def test_register_user(mock_get_users, mock_register_user, client, app):
    """测试注册新用户（成功）"""
    # 模拟用户名不存在
    mock_get_users.return_value = []
    new_user = {"id": 2, "username": "newuser", "email": "new@example.com"}
    mock_register_user.return_value = new_user

    response = client.post("/users", json={"username": "newuser", "password": "secret"})
    assert response.status_code == 200
    data = response.json()
...
@patch("Db.GetUsers")
def test_register_user_already_exists(mock_get_users, client, app):
    """测试注册时用户名已存在（400）"""
    mock_get_users.return_value = [{"id": 1, "username": "existing"}]

    response = client.post("/users", json={"username": "existing", "password": "secret"})
...
def test_update_user(mock_update_user, client, app):
    """测试更新用户（成功）"""
    updated = {"id": 1, "username": "updated", "email": "updated@example.com"}
